...
class Manager():
    def __init__(self, model, tail, datamanager, config, device, enable_log = False, callback = None, calib_head = None, calib_tail = None):
        self.model = model
        self.tail = tail
    ...
    def init_optimizer(self):
        ...
        if self.use_calib:
            self.calib_optimizer = torch.optim.Adam([
                ...
                {'params' : self.calib_tail.parameters()}],
                lr = self.config["LEARNING_RATE"], betas=(0.99, 0.999)
            )
    ...
    def train_loop(self, epoch):
        ...
        if self.calib_mode_on == False:
            ...
            for img, label, meta in iter(self.train_loader):
                ...
                calib_head_input = torch.concat(model_output, meta)
                calib_head_output = self.calib_head(calib_head_input)

                calib_tail_input = torch.concat(tail_output, calib_head_output)
    ...
    def save_best_model(self):
        ...
        torch.save(self.model, filename)
        filename = self.config["save_path"] + "/best_tail" + '.pth'
        torch.save(self.tail, filename)  

        if self.calib_mode_on:
            filename = self.config["save_path"] + "/best_calib_head" + '.pth'
            torch.save(self.calib_head, filename)
